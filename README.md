...
Most guides will tell you to install all the tools and start coding.

This is different.  
At each and every step of installations we'll stop and discuss __what's going on__.

Although we won't go into detail on _clojure the language_, we'll cover some basics too. 
Total and struggling beginners should enjoy this.

We'll end up with a little program for counting most used words in dev.to headlines.

We're only going to assume you're running a __recent version of Windows with PowerShell__<sup id="a1">[1](#f1)</sup> and have some programming experience. 

Follow along carefully, don't skip any parts, faithfully execute every command as I do!

Open up a PowerShell and let's get started...
...
# might take a minute or two to download. OpenJDK is almost exactly like Oracle's JDK, for our purposes there's no difference.
# The JDK includes all the necessary binaries to work with java, like java, javac, javah, javap and so on...
Writing web request ...
....
PS C:\Users\adas\clojure> Add-Type -AssemblyName System.IO.Compression.FileSystem # loads up .NET stuff for extracting Zips into PowerShell
...
...
PS C:\Users\adas\clojure> $javabin = Join-Path $PWD  "java11\bin" # create a path like C:\Users\adas\clojure\java11\bin

PS C:\Users\adas\clojure> $env:path+=";" + $javabin # temporarily put it on the path so we can access java globally
PS C:\Users\adas\clojure> java -version # did it work?
openjdk version "11.0.1" 318-1-16 # yes!
...
```
Great, `java` is ready.
...
So let's get a JAR for clojure.
 ...
 PS C:\Users\adas\clojure> [System.IO.Compression.ZipFile]::ExtractToDirectory((Join-Path $PWD "/clojure.jar"),(Join-Path $PWD "jar-disassembly")) 
 PS C:\Users\adas\clojure> # we extracted the jar to jar-disassembly, normally you don't ever manually extract a jar file
 PS C:\Users\adas\clojure> # but we really want to see what's inside
...
...
 PS C:\Users\adas\clojure> # interesting, mostly a lot of .class files
 PS C:\Users\adas\clojure> # this is JVM's (Java Virtual Machine) standard format for bytecode
 PS C:\Users\adas\clojure> # also note how everything is nested in directories
 PS C:\Users\adas\clojure> # as we'll later see directories are very important in the Java world
 PS C:\Users\adas\clojure> # to make use of our jar we need to put it on java's "classpath", it's like our systems PATH but for java
...
PS C:\Users\adas\clojure> java -jar clojure.jar # so if we do -jar instead, java will use automatically run Main-Class clojure.main
 ...
...
"value"
user=> ;; let's talk about symbols now
...
CompilerException java.lang.RuntimeException: Unable to resolve symbol: a-symbol in this context, compiling:(NO_SOURCE_PATH:0:0)
...
PS C:\Users\adas\clojure> ./main.clj #; should open main.clj in your main text editor
```
In `main.clj`:
```clojure
;;C:\users\adas\clojure\main.clj
(ns main)
(def nine 9)
```
...
## footnotes
<b id="f1">1</b> Let me know __in the comments__ if you want a Linux or MAC OS X take [↩](#a1)  
